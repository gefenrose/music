...
class MusicLibraryManager: ObservableObject {
    ...
    func loadLibrary() {
        isLoading = true
        Task {
            // buildLibrary() is nonisolated — runs on cooperative thread pool, not main actor
            let (allTracks, albumGroups, artistGroups) = await Self.buildLibrary()
    ...
    func importFiles(from urls: [URL]) {
        Task {
            // File I/O is nonisolated — runs off main thread
            await Self.copyFiles(urls)
            self.loadLibrary()
        }
    }
    ...
    // nonisolated = runs on cooperative thread pool, not main actor
    private static func buildLibrary() async -> ([Track], [AlbumGroup], [ArtistGroup]) {
        ...
        let localTracks = await scanLocalFiles()
    ...
    private static func copyFiles(_ urls: [URL]) async {
        for url in urls {
